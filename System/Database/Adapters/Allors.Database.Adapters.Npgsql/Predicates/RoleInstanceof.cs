...
namespace Allors.Database.Adapters.Npgsql
{
    ...
    internal sealed class RoleInstanceof : Predicate
    {
        ...
        internal override bool BuildWhere(ExtentStatement statement, string alias)
        {
            ...
            {
                statement.Append(" (" + statement.GetJoinName(this.role) + "." + ColumnNameForClass + " IS NOT NULL AND ");
                statement.Append(" " + statement.GetJoinName(this.role) + "." + ColumnNameForClass + "=" + statement.AddParameter(this.instanceClasses[0].Id) + ")");
            }
            else if (this.instanceClasses.Length > 1)
            {
                ...
                {
                    statement.Append(" (" + statement.GetJoinName(this.role) + "." + ColumnNameForClass + " IS NOT NULL AND ");
                    statement.Append(" " + statement.GetJoinName(this.role) + "." + ColumnNameForClass + "=" + statement.AddParameter(this.instanceClasses[i].Id) + ")");
