                ...
                {
                    {
                        ...
                        sql.Append("(\n");
                        sql.Append(ColumnNameForObject + " BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,\n");
                        sql.Append(ColumnNameForClass + " " + Mapping.SqlTypeForClass + ",\n");
                        sql.Append(ColumnNameForVersion + " " + Mapping.SqlTypeForVersion + "\n");
                        sql.Append(")\n");
                    ...
                    foreach (var @class in this.mapping.Database.MetaPopulation.DatabaseClasses)
                    {
                        ...
                        sql.Append("(\n");
                        sql.Append(ColumnNameForObject + " " + Mapping.SqlTypeForObject + " PRIMARY KEY,\n");
                        sql.Append(ColumnNameForClass + " " + Mapping.SqlTypeForClass);
                    ...
                    foreach (var relationType in this.mapping.Database.MetaPopulation.DatabaseRelationTypes)
                    {
                        ...
                        if (!roleType.ObjectType.IsUnit && ((associationType.IsMany && roleType.IsMany) || !relationType.ExistExclusiveDatabaseClasses))
                        {
                            ...
                            var constraintContent = relationType.RoleType.IsOne
                                ? $"PRIMARY KEY ({ColumnNameForAssociation})"
                                : $"PRIMARY KEY ({ColumnNameForAssociation}, {ColumnNameForRole})";
                            var primaryKeyName = Mapping.HashIdentifier("pk_" + relationType.Id.ToString("N") + constraintContent);

                            var sql =
$@"CREATE TABLE {tableName}(
    {ColumnNameForAssociation} {Mapping.SqlTypeForObject},
    {ColumnNameForRole} {Mapping.SqlTypeForObject},
    CONSTRAINT {primaryKeyName} {constraintContent}
                ...
                try
                {
                    foreach (var @class in this.mapping.Database.MetaPopulation.DatabaseClasses)
                    {
                        ...
                        foreach (var associationType in @class.DatabaseAssociationTypes)
                        {
                            ...
                            if (relationType.IsIndexed)
                            {
                                ...
                                if (!(associationType.IsMany && roleType.IsMany) && relationType.ExistExclusiveDatabaseClasses && roleType.IsMany)
                                {
                                    var indexContent = $"ON {tableName} ({this.mapping.ColumnNameByRelationType[relationType]})";
                                    var indexName = Mapping.HashIdentifier("idx_" + relationType.Id.ToString("N") + indexContent);
                    ...
                    foreach (var relationType in this.mapping.Database.MetaPopulation.DatabaseRelationTypes)
                    {
                        if (relationType.IsIndexed)
                        {
                            ...
                            {
                                var tableName = this.mapping.TableNameForRelationByRelationType[relationType];
                                var indexContent = $"ON {tableName} ({ColumnNameForRole})";
                                var indexName = Mapping.HashIdentifier("idx_" + relationType.Id.ToString("N") + indexContent);
                                var sql = new StringBuilder();
                                sql.Append("CREATE INDEX " + indexName + "\n");
                                sql.Append(indexContent);
                                using (var command = new NpgsqlCommand(sql.ToString(), connection))
                                {
                                    command.ExecuteNonQuery();
                                }
