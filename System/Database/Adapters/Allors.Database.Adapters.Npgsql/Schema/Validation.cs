...
namespace Allors.Database.Adapters.Npgsql
{
    ...
    public class Validation
    {
        ...
        private void Validate()
        {
            ...
            {
                if (objectsTable.ColumnByName.Count != 3)
                {
                    this.InvalidTables.Add(objectsTable);
                }

                this.ValidateColumn(objectsTable, ColumnNameForObject, Mapping.SqlTypeForObject);
                this.ValidateColumn(objectsTable, ColumnNameForClass, Mapping.SqlTypeForClass);
                this.ValidateColumn(objectsTable, ColumnNameForVersion, Mapping.SqlTypeForVersion);
            }
            ...
            foreach (var @class in this.Database.MetaPopulation.DatabaseClasses)
            {
                ...
                {
                    this.ValidateColumn(table, ColumnNameForObject, Mapping.SqlTypeForObject);
                    this.ValidateColumn(table, ColumnNameForClass, Mapping.SqlTypeForClass);
            ...
            foreach (var relationType in this.Database.MetaPopulation.DatabaseRelationTypes)
            {
                    ...
                    ((associationType.IsMany && roleType.IsMany) || !relationType.ExistExclusiveDatabaseClasses))
                {
                    ...
                    {
                        if (table.ColumnByName.Count != 2)
                        {
                            this.InvalidTables.Add(table);
                        }

                        this.ValidateColumn(table, ColumnNameForAssociation, Mapping.SqlTypeForObject);
                        ...
                        this.ValidateColumn(table, ColumnNameForRole, roleSqlType);
                    }
                }
            }
        ...
        {
            var objectColumn = table.GetColumn(StripQuotes(columnName));
