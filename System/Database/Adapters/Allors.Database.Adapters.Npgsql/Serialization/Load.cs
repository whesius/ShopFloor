...
namespace Allors.Database.Adapters.Npgsql.Npgsql
{
    ...
    public class Load
    {
        ...
        private void LoadObjectsDatabase(XmlReader reader)
        {
            ...
            using (var writer = this.connection.BeginBinaryImport($"COPY {mapping.TableNameForObjects} ({ColumnNameForObject}, {ColumnNameForClass}, {ColumnNameForVersion}) FROM STDIN (FORMAT BINARY)"))
            {
...
where c = '{@class.Id}'";
        ...
        private void LoadUnitRelations(XmlReader reader, IRelationType relationType)
        {
            ...
            try
            {
                foreach (var kvp in unitRelationsByClass)
                {
                    ...
                    var functionName = this.database.Mapping.ProcedureNameForSetUnitRoleByRelationTypeByClass[@class][relationType];
        ...
        private void LoadCompositeRelations(XmlReader reader, IRelationType relationType)
        {
            ...
            try
            {
                ...
                var functionName = relationType.RoleType.IsOne ?
