...
namespace Allors.Database.Adapters.Npgsql
{
    ...
    public class Strategy : IStrategy
    {
        ...
        public virtual void SetUnitRole(IRoleType roleType, object unit)
        {
            this.AssertExist();
            roleType.UnitRoleChecks(this);
            unit = roleType.Normalize(unit);

            this.SetUnitRoleInternal(roleType, unit);
        }
        ...
        public virtual void SetCompositeRole(IRoleType roleType, IObject newRole)
        {
            ...
            else
            {
                ...
                var newRoleStrategy = (Strategy)newRole.Strategy;

                if (roleType.AssociationType.IsOne)
                {
                    this.SetCompositeRoleOne2One(roleType, newRoleStrategy);
                }
                else
                {
                    this.SetCompositeRoleMany2One(roleType, newRoleStrategy);
                }
            }
        }
        ...
        public virtual void AddCompositesRole(IRoleType roleType, IObject role)
        {
            this.AssertExist();
            ...
            roleType.CompositeRolesChecks(this, role);
            var roleStrategy = (Strategy)role.Strategy;

            if (roleType.AssociationType.IsOne)
            {
                this.AddCompositesRoleOne2Many(roleType, roleStrategy);
            }
            else
            {
                this.AddCompositesRoleMany2Many(roleType, roleStrategy);
            }
        }

        public virtual void RemoveCompositesRole(IRoleType roleType, IObject role)
        {
            this.AssertExist();
