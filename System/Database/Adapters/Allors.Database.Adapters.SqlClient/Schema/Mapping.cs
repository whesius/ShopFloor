...
namespace Allors.Database.Adapters.SqlClient
{
    ...
    {
        public string ParamInvocationFormat => ParamFormat;

        public string ParamInvocationNameForObject => this.ParamNameForObject;
        public string ParamInvocationNameForClass => this.ParamNameForClass;
        ...
        public IDictionary<IRoleType, string> ParamInvocationNameByRoleType => this.ParamNameByRoleType;

        public IDictionary<IClass, string> ProcedureNameForDeleteObjectByClass =>
            this.procedureNameForDeleteObjectByClass;

        public IDictionary<IClass, string> ProcedureNameForCreateObjectsByClass =>
            this.procedureNameForCreateObjectsByClass;

        public IDictionary<IClass, string> ProcedureNameForGetUnitRolesByClass =>
            this.procedureNameForGetUnitRolesByClass;

        public IDictionary<IClass, IDictionary<IRelationType, string>>
            ProcedureNameForSetUnitRoleByRelationTypeByClass => this.procedureNameForSetUnitRoleByRelationTypeByClass;

        public IDictionary<IRelationType, string> ProcedureNameForGetRoleByRelationType =>
            this.procedureNameForGetRoleByRelationType;

        public IDictionary<IRelationType, string> ProcedureNameForSetRoleByRelationType =>
            this.procedureNameForSetRoleByRelationType;

        public IDictionary<IRelationType, string> ProcedureNameForAddRoleByRelationType =>
            this.procedureNameForAddRoleByRelationType;

        public IDictionary<IRelationType, string> ProcedureNameForRemoveRoleByRelationType =>
            this.procedureNameForRemoveRoleByRelationType;

        public IDictionary<IRelationType, string> ProcedureNameForClearRoleByRelationType =>
            this.procedureNameForClearRoleByRelationType;

        public IDictionary<IRelationType, string> ProcedureNameForGetAssociationByRelationType =>
            this.procedureNameForGetAssociationByRelationType;

        public IDictionary<IClass, string> ProcedureNameForCreateObjectByClass =>
            this.procedureNameForCreateObjectByClass;

        public string ProcedureNameForInstantiate { get; }

        public string ProcedureNameForGetVersion { get; }

        public string ProcedureNameForUpdateVersion { get; }

        public IDictionary<IClass, string> ProcedureNameForPrefetchUnitRolesByClass =>
            this.procedureNameForPrefetchUnitRolesByClass;

        public IDictionary<IRelationType, string> ProcedureNameForPrefetchRoleByRelationType =>
            this.procedureNameForPrefetchRoleByRelationType;

        public IDictionary<IRelationType, string> ProcedureNameForPrefetchAssociationByRelationType =>
            this.procedureNameForPrefetchAssociationByRelationType;

        public string ParamInvocationNameForAssociation => this.ParamNameForAssociation;

        public string ParamInvocationNameForCompositeRole => this.ParamNameForCompositeRole;

        public string ParamInvocationNameForCount => this.ParamNameForCount;

        public string ObjectArrayParamInvocationName => string.Empty;

        public string CompositeRoleArrayParamInvocationName => string.Empty;

        public string StringCollation => "COLLATE Latin1_General_100_BIN2";
        public string Ascending => "ASC";
        public string Descending => "DESC";

        public IDictionary<IRelationType, string> TableNameForRelationByRelationType =>
            this.tableNameForRelationByRelationType;
        ...
        private readonly IDictionary<IClass, IDictionary<IRelationType, string>>
            procedureNameForSetUnitRoleByRelationTypeByClass;
            ...
            foreach (var relationType in database.MetaPopulation.DatabaseRelationTypes)
            {
                ...
                if (roleType.ObjectType.IsUnit && ((IUnit)roleType.ObjectType).IsDecimal)
                {
...

                    {
                        ...
                        if (!this.TableTypeNameForDecimalRelationByScaleByPrecision.TryGetValue(precision,
                                out var decimalRelationTableByScale))
                        {
                            decimalRelationTableByScale = new Dictionary<int, string>();
                            this.TableTypeNameForDecimalRelationByScaleByPrecision[precision] =
                                decimalRelationTableByScale;
                        }
...

                    {
                        ...
                        if (!this.TableTypeNameForDecimalInByScaleByPrecision.TryGetValue(precision,
                                out var decimalInTableByScale))
                        {
...
";
            ...
            {
                this.tableNameForObjectByClass.Add(@class,
                    $"{this.Database.SchemaName}.{this.NormalizeName(@class.SingularName)}");
                ...
                foreach (var roleType in @class.DatabaseRoleTypes)
                {
                    ...
                    else if (!(associationType3.IsMany && roleType.IsMany) &&
                             relationType.ExistExclusiveDatabaseClasses && !roleType.IsMany)
                    {
                        this.columnNameByRelationType[relationType] = this.NormalizeName(roleType.SingularName);
                    }
                }
            ...
            foreach (var relationType in this.Database.MetaPopulation.DatabaseRelationTypes)
            {
                ...
                if (!roleType.ObjectType.IsUnit && ((associationType.IsMany && roleType.IsMany) ||
                                                    !relationType.ExistExclusiveDatabaseClasses))
                {
            ...
            this.procedureNameForSetUnitRoleByRelationTypeByClass =
                new Dictionary<IClass, IDictionary<IRelationType, string>>();
            ...
            foreach (var @class in this.Database.MetaPopulation.DatabaseClasses)
            {
                ...
                foreach (var associationType in @class.DatabaseAssociationTypes)
                {
                    if (!(associationType.IsMany && associationType.RoleType.IsMany) &&
                        associationType.RelationType.ExistExclusiveDatabaseClasses && associationType.RoleType.IsMany)
                    {
                ...
                foreach (var roleType in @class.DatabaseRoleTypes)
                {
                    if (roleType.ObjectType.IsUnit)
                    {
                        this.SetUnitRoleType(roleType, @class);
                             ...
                             roleType.RelationType.ExistExclusiveDatabaseClasses && roleType.IsOne)
                    {
            ...
            foreach (var relationType in this.Database.MetaPopulation.DatabaseRelationTypes)
            {
                if (!relationType.RoleType.ObjectType.IsUnit &&
                    ((relationType.AssociationType.IsMany && relationType.RoleType.IsMany) ||
                     !relationType.ExistExclusiveDatabaseClasses))
                {
                    this.procedureNameForPrefetchAssociationByRelationType.Add(relationType,
                        $"{this.Database.SchemaName}.{ProcedurePrefixForPrefetchAssociation}{relationType.Tag}");
                    this.procedureNameForClearRoleByRelationType.Add(relationType,
                        $"{this.Database.SchemaName}.{ProcedurePrefixForClearRole}{relationType.Tag}");
        ...
        public string GetTableTypeNameForIn(IRoleType roleType)
        {
            ...
            return unitTypeTag switch
            {
                ...
                UnitTags.Decimal => this.TableTypeNameForDecimalInByScaleByPrecision[roleType.Precision.Value][
                    roleType.Scale.Value],
                _ => throw new ArgumentException($"Unknown Unit ObjectType: {unitTypeTag}")
            };
        }
        ...
        public string RoleJoinAlias(IRoleType role) => role.SingularFullName + "_R";

        public string AssociationJoinAlias(IAssociationType association) => association.SingularFullName + "_A";

        public string RoleInstanceJoinAlias(IRoleType role) => role.SingularFullName + "_RC";

        public string AssociationInstanceJoinAlias(IAssociationType association) => association.SingularFullName + "_AC";
        ...
        private void LoadObjects(IClass @class)
        {
            ...
            var name = $"{this.Database.SchemaName}.{ProcedurePrefixForLoad}{@class.Tag}";
...
END";
        ...
        {
            var table = this.tableNameForObjectByClass[@class];
            var name = $"{this.Database.SchemaName}.{ProcedurePrefixForCreateObject}{@class.Tag}";
...
END";
        ...
        private void CreateObjects(IClass @class)
        {
            ...
            var name = $"{this.Database.SchemaName}.{ProcedurePrefixForCreateObjects}{@class.Tag}";
...
END";
        ...
        {
            var table = this.tableNameForObjectByClass[@class.ExclusiveDatabaseClass];
            var name = $"{this.Database.SchemaName}.{ProcedurePrefixForDeleteObject}{@class.Tag}";
...
END";
        ...
        private void GetUnitRoles(IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class.ExclusiveDatabaseClass];
            var name = $"{this.Database.SchemaName}.{ProcedurePrefixForGetUnits}{@class.Tag}";
...
";
        ...
        private void PrefetchUnitRoles(IClass @class)
        {
            var sortedUnitRoleTypes = this.Database.GetSortedUnitRolesByObjectType(@class);
            var table = this.tableNameForObjectByClass[@class.ExclusiveDatabaseClass];
            var name = $"{this.Database.SchemaName}.{ProcedurePrefixForPrefetchUnits}{@class.Tag}";
...
END";
        ...
        private void GetCompositesRoleObjectTable(IAssociationType associationType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForGetRole}{relationType.Tag}";
...
END";
        ...
        private void PrefetchCompositesRoleObjectTable(IAssociationType associationType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForPrefetchRole}{relationType.Tag}";
...
END";
        ...
        private void GetCompositeAssociationObjectTable(IAssociationType associationType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForGetAssociation}{@class.Tag}_{relationType.Tag}";
...
END";
        ...
        private void PrefetchCompositeAssociationObjectTable(IAssociationType associationType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForPrefetchAssociation}{@class.Tag}_{relationType.Tag}";
...
END";
        ...
        private void AddCompositeRoleObjectTable(IAssociationType associationType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForAddRole}{@class.Tag}_{relationType.Tag}";
...
END";
        ...
        private void RemoveCompositeRoleObjectTable(IAssociationType associationType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForRemoveRole}{@class.Tag}_{relationType.Tag}";
...
END";
        ...
        private void ClearCompositeRoleObjectTable(IAssociationType associationType, IClass @class)
        {
            var relationType = associationType.RelationType;
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForClearRole}{@class.Tag}_{relationType.Tag}";
...
END";
        ...
        {
            if (!this.procedureNameForSetUnitRoleByRelationTypeByClass.TryGetValue(@class,
                    out var procedureNameForSetUnitRoleByRelationType))
            {
                procedureNameForSetUnitRoleByRelationType = new Dictionary<IRelationType, string>();
                this.procedureNameForSetUnitRoleByRelationTypeByClass.Add(@class,
                    procedureNameForSetUnitRoleByRelationType);
            }
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForSetRole}{@class.Tag}_{roleType.RelationType.Tag}";
...
END";
        ...
        private void GetCompositeRoleObjectTable(IRoleType roleType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForGetRole}{roleType.RelationType.Tag}";
            this.procedureNameForGetRoleByRelationType.Add(relationType, name);
...
END";
        ...
        private void PrefetchCompositeRoleObjectTable(IRoleType roleType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForPrefetchRole}{roleType.RelationType.Tag}";
            this.procedureNameForPrefetchRoleByRelationType.Add(relationType, name);
...
END";
        ...
        private void GetCompositeAssociationOne2OneObjectTable(IRoleType roleType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForGetAssociation}{@class.Tag}_{roleType.RelationType.Tag}";
            this.procedureNameForGetAssociationByRelationType.Add(relationType, name);
...
END";
        ...
        private void PrefetchCompositeAssociationObjectTable(IRoleType roleType, IClass @class)
        {
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForPrefetchAssociation}{@class.Tag}_{roleType.RelationType.Tag}";
...
END";
        ...
        private void ClearCompositeRole(IRoleType roleType, IClass @class)
        {
            ...
            var table = this.tableNameForObjectByClass[@class];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForClearRole}{@class.Tag}_{roleType.RelationType.Tag}";
            this.procedureNameForClearRoleByRelationType.Add(relationType, name);
...
END";
        ...
        private void AddCompositeRoleRelationTable(IRelationType relationType)
        {
            var table = this.tableNameForRelationByRelationType[relationType];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForAddRole}{relationType.Tag}";
            this.procedureNameForAddRoleByRelationType.Add(relationType, name);
...
END";
        ...
        private void RemoveCompositeRoleRelationTable(IRelationType relationType)
        {
            var table = this.tableNameForRelationByRelationType[relationType];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForRemoveRole}{relationType.Tag}";
            this.procedureNameForRemoveRoleByRelationType.Add(relationType, name);
...
END";
        ...
        private void SetCompositeRoleRelationType(IRelationType relationType)
        {
            var table = this.tableNameForRelationByRelationType[relationType];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForSetRole}{relationType.Tag}";
            this.procedureNameForSetRoleByRelationType.Add(relationType, name);
...
END";
        ...
        private void GetCompositeAssociationRelationTable(IRelationType relationType)
        {
            var table = this.tableNameForRelationByRelationType[relationType];
            var name =
                $"{this.Database.SchemaName}.{ProcedurePrefixForGetAssociation}{relationType.Tag}";
            this.procedureNameForGetAssociationByRelationType.Add(relationType, name);
