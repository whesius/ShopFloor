...
namespace Allors.Database.Meta.Configuration
{
    ...
    public sealed partial class RelationType : IRelationTypeBase
    {
        ...
        public string SchemaName  => this.AssociationType.ObjectType.SchemaName + "_" + this.RoleType.SchemaName;
        ...
        public override bool Equals(object obj) => this.Id.Equals((obj as RelationType)?.Id);
        ...
        public int CompareTo(object obj) => this.Id.CompareTo((obj as RelationType)?.Id);
        ...
        public void Validate(ValidationLog log)
        {
            ...
            {
                if (log.ExistRelationName(this.Name))
                {
                    var message = "name of " + this.ValidationName + " is already in use";
                    log.AddError(message, this, ValidationKind.Unique, "RelationType.Name");
                }
                else
                {
                    log.AddRelationTypeName(this.Name);
                }

                if (log.ExistRelationName(this.ReverseName))
                {
                ...
                {
                    log.AddRelationTypeName(this.ReverseName);
                }

                if (log.ExistObjectTypeName(this.Name))
                {
                    var message = "name of " + this.ValidationName + " is in conflict with object type " + this.Name;
                    log.AddError(message, this, ValidationKind.Unique, "RelationType.Name");
                }

                if (log.ExistObjectTypeName(this.ReverseName))
                {
            ...
            else if (this.AssociationType == null)
            {
                ...
                log.AddError(message, this, ValidationKind.Required, "RelationType.AssociationType");
            }
            else
            {
                var message = this.ValidationName + " has no role type";
                log.AddError(message, this, ValidationKind.Required, "RelationType.RoleType");
            }

            this.AssociationType?.Validate(log);

            this.RoleType?.Validate(log);
        }
    }
}
