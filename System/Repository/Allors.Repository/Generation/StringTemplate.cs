...
namespace Allors.Repository.Generation
{
    ...
    public class StringTemplate
    {
        ...
        public StringTemplate(FileInfo fileInfo)
        {
            ...
            {
                throw new InvalidOperationException("Template file not found: " + fileInfo.FullName);
            }

            var templateGroup = new TemplateGroupFile(this.fileInfo.FullName, '$', '$');
            ...
            {
                throw new InvalidOperationException("Template has no id");
            }
        }
        ...
        public void Generate(Repository repository, DirectoryInfo outputDirectory)
        {
            ...
            configurationXml.LoadXml(configurationTemplate.Render(CultureInfo.InvariantCulture));
            ...
            foreach (XmlElement generation in configurationXml.DocumentElement.SelectNodes(GenerationKey))
            {
                ...
                var result = template.Render(CultureInfo.InvariantCulture);
                location.Save(output, result);
            }
        }

        private static string Render(TemplateGroupFile templateGroup, string templateName)
        {
            var template = templateGroup.GetInstanceOf(templateName);
            if (template != null)
            {
        ...
        private sealed class LogAdapter : ITemplateErrorListener
        {
